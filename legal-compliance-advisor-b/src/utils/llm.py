"""Utility helpers for resilient Gemini calls in ADK-B."""
...
def generate_with_retry(
    ...
    if enable_cache:
        ...
        if cache_key in RESPONSE_CACHE:
            logging.info("ADK-B cache hit for prompt (key: %s)", cache_key)
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            logging.warning(
                "ADK-B LLM call failed (attempt %s/%s): %s. Retrying in %.1fs",
                attempt,
                attempts,
                exc,
                sleep_for,
